...
const Body = () => {
  ...
  async function fetchData() {
    ...
    console.log(data, "swiggyDaksadjfk");
    setCacheData(
  ...
  const RestarantCardPromoted = withPromotedComponent(RestaurantCard);
            ...
            swiggyData?.map((item, index) => {
              return (
                <Fragment key={item?.info?.id}>
                  {true ? (
                    <RestarantCardPromoted foodItem={item?.info} />
                  ) : (
